...
app.post("/api/auth/register", async (req, res) => {
  ...
  if (!name || !email || !password || !role) {
    return res
      .status(400)
      .json({ message: "Nama, email, password, dan role wajib diisi!" });
  }
...
// ==========================================
// 2. API ENDPOINT LOGIN (SUDAH DIPERBAIKI FOTONYA)
// ==========================================
...
app.get("/api/admin/dashboard", async (req, res) => {
  try {
    // 1. Hitung Total UMKM
    const [totalUmkmRow] = await db.query(
      "SELECT COUNT(*) as total FROM umkm_profiles",
    );

    // 2. Hitung Total Produk
    const [totalProdukRow] = await db.query(
      "SELECT COUNT(*) as total FROM products",
    );

    // 3. Hitung UMKM yang berstatus 'Aktif'
    const [activeUmkmRow] = await db.query(
      "SELECT COUNT(*) as total FROM umkm_profiles WHERE status = 'Aktif'",
    );
    ...
    return res
      .status(500)
      .json({ message: "Gagal memuat data statistik admin." });
  }
});
...
app.get("/api/admin/umkm", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Gagal mengambil seluruh data UMKM." });
  }
});
...
app.get("/api/admin/products", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Gagal mengambil data produk admin." });
  }
});
  ...
  try {
    ...
    return res.status(200).json({
      stats: {
        ...
        umkmAktif,
        tingkatAktif:
          totalUMKM > 0 ? ROUND_TO_ONE((umkmAktif / totalUMKM) * 100) : 0,
        rataRataProduk:
          totalUMKM > 0 ? ROUND_TO_ONE(totalProduk / totalUMKM) : 0,
    ...
    return res
      .status(500)
      .json({ message: "Gagal menyusun laporan database." });
  }
});
...
app.get("/api/umkm/products/:userId", async (req, res) => {
  try {
    ...
    const [umkmRow] = await db.query(
      "SELECT id, business_name FROM umkm_profiles WHERE user_id = ?",
      [userId],
    );
    ...
    if (umkmRow.length === 0) {
      return res
        .status(404)
        .json({ message: "Profil UMKM belum didaftarkan oleh Admin." });
    }
    ...
    const [products] = await db.query(
      "SELECT * FROM products WHERE umkm_id = ? ORDER BY id DESC",
      [umkmId],
    );

    // 3. Hitung ringkasan total stok barang toko mereka
    const [stokRow] = await db.query(
      "SELECT SUM(stock) as total_stok FROM products WHERE umkm_id = ?",
      [umkmId],
    );
    ...
    return res
      .status(500)
      .json({ message: "Gagal memuat inventori toko Anda." });
  }
});
...
app.post("/api/umkm/products", async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ message: "Produk dagangan Anda berhasil ditambahkan!" });
  } catch (error) {
    console.error(error);
...
app.get("/api/umkm/dashboard-full/:userId", async (req, res) => {
  try {
    ...
    const [totalProd] = await db.query(
      "SELECT COUNT(*) as total FROM products WHERE umkm_id = ?",
      [umkmId],
    );

    // 3. Hitung Total Penjualan (Rp) & Total Transaksi (Count)
    const [salesRow] = await db.query(
      "SELECT SUM(total) as total_rp, COUNT(*) as total_count FROM orders WHERE umkm_id = ?",
      [umkmId],
    );
    ...
    return res
      .status(500)
      .json({ message: "Gagal sinkronisasi database mitra." });
  }
});
...
app.post("/api/umkm/orders-manual", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Gagal memproses transaksi kasir." });
  }
});
...
app.post("/api/admin/products", async (req, res) => {
  const { title, category_name, price, stock, business_name, image_url } =
    req.body;
  ...
  try {
    ...
    const [umkmRow] = await db.query(
      "SELECT id FROM umkm_profiles WHERE business_name = ?",
      [business_name],
    );
...
app.post("/api/admin/umkm", async (req, res) => {
  ...
  if (!business_name || !owner_name || !email) {
    return res
      .status(400)
      .json({ message: "Nama usaha, nama pemilik, dan email wajib diisi!" });
  }
  ...
  try {
    ...
    if (existingUser.length > 0) {
      return res
        .status(400)
        .json({ message: "Email pemilik sudah terdaftar di sistem!" });
    }
...
app.get("/api/admin/categories", async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      'SELECT DISTINCT category as name FROM umkm_profiles WHERE category IS NOT NULL AND category != ""',
    );
...
app.get("/api/umkm/profile/:userId", async (req, res) => {
  try {
    const { userId } = req.params;
    const [rows] = await db.query(
      "SELECT * FROM umkm_profiles WHERE user_id = ?",
      [userId],
    );
...
app.put("/api/umkm/profile/:userId", async (req, res) => {
  try {
    ...
    const { business_name, owner_name, phone, location, category, logo_url } =
      req.body;

    if (!business_name || !owner_name) {
      return res
        .status(400)
        .json({ message: "Nama toko dan nama pemilik wajib diisi!" });
    }
    ...
    return res
      .status(200)
      .json({ message: "Profil toko berhasil diperbarui!" });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
      .json({ message: "Gagal memperbarui profil toko di database." });
  }
});
