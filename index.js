...
const jwt = require('jsonwebtoken'); // 1. Tambahkan import jwt di sini
...
app.get('/api/umkm/dashboard-full/:userId', async (req, res) => {
    try {
        ...
        // 1. Ambil Profil UMKM berdasarkan User ID login
        const [umkmRow] = await db.query('SELECT id, business_name FROM umkm_profiles WHERE user_id = ?', [userId]);
        
        // 💡 PERBAIKAN: Jika ID user yang mengakses adalah Admin (bukan UMKM), berikan response kosong yang aman agar tidak melempar 404 Not Found
        if (umkmRow.length === 0) {
            return res.status(200).json({
